import React from 'react';
import { useNavigate } from 'react-router-dom';
import GoogleDriveSyncSettings from '../components/GoogleDriveSyncSettings';
import './GoogleDriveSettingsPage.css';

const GoogleDriveSettingsPage = () => {
  const navigate = useNavigate();

  const handleBack = () => {
    navigate('/profile');
  };

  return (
    <div className="google-drive-settings-page">
      <div className="google-drive-settings-page__header">
        <button
          className="google-drive-settings-page__back-button"
          onClick={handleBack}
          aria-label="Go back"
        >
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M15 18l-6-6 6-6" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </button>
        <h1 className="google-drive-settings-page__title">Google Drive Backup</h1>
      </div>

      <GoogleDriveSyncSettings />
    </div>
  );
};

export default GoogleDriveSettingsPage;
